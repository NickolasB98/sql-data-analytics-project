...
-- Base Query: Retrieves core columns from tables
    SELECT 
        ...
        c.birthdate,
        (EXTRACT(YEAR FROM AGE(CURRENT_DATE, c.birthdate)) + 1) AS age
 ...
 SELECT
    ...
    total_products,
    lifespan,
    -- average order value (AOV) = total sales / total n of orders
    CASE 
        WHEN total_orders = 0 THEN 0
        ELSE total_sales / total_orders 
    END AS avg_order_value,
    -- average monthly spending = total sales / n of months
    -- n of months = lifespan (period a customer is active)
    -- lifespand = from the customer's first order to the last order
    CASE 
